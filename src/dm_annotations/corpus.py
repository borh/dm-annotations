...
def add_annotations(doc, spans, key="sc"):
    # print(doc, spans, key, doc.spans.keys())
...
def add_regex_span(doc, regex, label, key="sc", reverse_find=False):
    ...
    try:
        ...
        logging.error(f"'{regex}' not valid regex")
        return None
...
def read_annotations_excel(path, model="ja_core_news_sm"):
    ...
    for r in df.to_dict("records"):
        ...
        doc._.title = title
        add_regex_span(doc, r["connective"], "manual_connective", key="connective")
        add_regex_span(doc, r["modality"], "manual_modality", key="modality")
...
def merge_contiguous_spans(doc, spans):
    # Initialize an empty list to store the merged spans
    merged_spans = []
